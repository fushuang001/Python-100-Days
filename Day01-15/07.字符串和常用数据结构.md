* [字符串和常用数据结构](#字符串和常用数据结构)
   ...
   * [使用字典](#使用字典)
   * [练习](#练习)
      * [练习1：在屏幕上显示跑马灯文字。](#练习1在屏幕上显示跑马灯文字)
      * [练习2：设计一个函数产生指定长度的验证码，验证码由大小写字母和数字构成。](#练习2设计一个函数产生指定长度的验证码验证码由大小写字母和数字构成)
      * [练习3：设计一个函数返回给定文件名的后缀名。](#练习3设计一个函数返回给定文件名的后缀名)
      * [练习4：设计一个函数返回传入的列表中最大和第二大的元素的值。](#练习4设计一个函数返回传入的列表中最大和第二大的元素的值)
      * [练习5：计算指定的年月日是这一年的第几天。](#练习5计算指定的年月日是这一年的第几天)
      * [练习6：打印<a href="https://zh.wikipedia.org/wiki/杨辉三角形" rel="nofollow">杨辉三角</a>。](#练习6打印杨辉三角)
   * [综合案例](#综合案例)
      * [案例1：双色球选号。](#案例1双色球选号)
      * [综合案例2：<a href="https://zh.wikipedia.org/wiki/约瑟夫斯问题" rel="nofollow">约瑟夫环问题</a>。](#综合案例2约瑟夫环问题)
      * [综合案例3：<a href="https://zh.wikipedia.org/wiki/井字棋" rel="nofollow">井字棋</a>游戏。](#综合案例3井字棋游戏)
...
for val in f:
    print(val)
# 下面方式把 for val in f 输出到 list
f22 = []
f2 = (x ** 2 for x in range(1,9))
...
print(t[0])
print(t[3])
# 元组最常用的功能是和 %s, %d 相关的 print 语句；
# 不过既然这篇 GitHub 提供了更优美的 format & f 方式，就用更简单的了
t = ('Forest', "Cisco", 6)
print(f'{t[0]} has been working for {t[1]} for about {t[2]} years.')
# 遍历元组中的值
for member in t:
    print(member)
# 重新给元组赋值
...
print(scores['骆昊'])
print(scores['狄仁杰'])
# 对字典中所有键值对进行遍历, 输出结果类型于 狄仁杰: 82
for key in scores:
    print(f'{key}: {scores[key]}')
# python3 还可以使用 dict.items 同时遍历出 dict 里面的 key & value;  输出类似于 狄仁杰 82
# python2 是 dict.iteritems
...
def max2(x):
    ...
    for index in range(2, len(x)):
        if x[index] > m1:
            # 注意先后顺序。如果 m1 = x[index] 在前，m2 = m1 也会是最大值
            m2 = m1
            m1 = x[index]
        elif x[index] > m2:
            m2 = x[index]
...
ur_list = [1,2,3,4,5,4,3,2,7,8,9,10]
...
def main():
    # 占位符，list 里面 30 个元素表示 30 个人
    persons = [True] * 30
    # counter - 游戏进行次数；index - person 站位； number - 报数；
    counter, index, number = 0, 0, 0
    # 一次 counter 扔海里一个，15 次即可；
    while counter < 15:
        # 等同于 if True, 每个人默认都是 True; 被修改为 False 的无法继续游戏
        if persons[index]:
            number += 1
            if number == 9:
                # 倒霉蛋产生，True -> False, 并且 counter +1, 重置 number
                persons[index] = False
                counter += 1
                number = 0
        index += 1
        # index = index % 30, 只是目前不理解含义
        # 若不写 index %= 30, if persons[index] 会报错，index out of range
        index %= 30
...
```
[将 Python 执行过程演示出来](http://www.codeskulptor.org/viz/index.html)

index 在上面 code 中最大值为 23，并非 29
![index out of range](./res/code-viz.png)
