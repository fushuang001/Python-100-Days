* [面向对象编程基础](#面向对象编程基础)
   ...
   * [面向对象的支柱](#面向对象的支柱)
   * [练习](#练习)
      * [练习1：定义一个类描述数字时钟。](#练习1定义一个类描述数字时钟)
      * [练习2：定义一个类描述平面上的点并提供移动点和计算到另一个点距离的方法。](#练习2定义一个类描述平面上的点并提供移动点和计算到另一个点距离的方法)
...
class Student(object):
    ...
    def __init__(self, name, age):  # <<<< 1. 静态特征(属性)
        self.name = name
        self.age = age

    def study(self, course_name):   # <<<< 2. 动态特征(行为)
        print('%s正在学习%s.' % (self.name, course_name))

    # PEP 8要求标识符的名字用全小写多个单词用下划线连接
    # 但是部分程序员和公司更倾向于使用驼峰命名法(驼峰标识)
    def watch_movie(self):          # 类的方法和普通函数的区别，需要有一个额外的第一个参数名称，通常是 self
        if self.age < 18:
...
def main():
    ...
    while True:
        print(clock.show())   # 封装，隐藏了细节，只需要知道 clock.show 是显示时间即可
        sleep(1)
        clock.run()           # 封装，隐藏了细节，只需要知道 clock.run 是模拟 时钟走字 即可
